...
pub fn verify_conjunction_introduction(assumptions: &Vec<Proposition>, conclusion: &Proposition) -> Result<(),VerificationError> {
    ...
    let [conjunction_head, conjunction_left, conjunction_right] = TupleOrError::prop_as_slice(conclusion)? else { return Err(VerificationError::InvalidStepSpecification) };
