...
use crate::{
    proofs::{
        ...
        sequential::{
            SequentialProof,
            at_path_enum::{OwnedSequentialProofAtPathEnum, SequentialProofAtPathEnum},
            subproofs::immediate::ImmediateSequentialProofInProofPath,
        },
    },
...
mod from {
    use crate::proofs::sequential::subproofs::{
        SequentialProofInProofPath, immediate::ImmediateSequentialProofInProofPath,
    };

    impl From<ImmediateSequentialProofInProofPath> for SequentialProofInProofPath {
        fn from(value: ImmediateSequentialProofInProofPath) -> Self {
            Self(vec![value])
        }
    }
    impl<const N: usize> From<[ImmediateSequentialProofInProofPath; N]> for SequentialProofInProofPath {
        fn from(values: [ImmediateSequentialProofInProofPath; N]) -> Self {
            Self(values.into())
        }
    }
    impl From<Box<[ImmediateSequentialProofInProofPath]>> for SequentialProofInProofPath {
        fn from(values: Box<[ImmediateSequentialProofInProofPath]>) -> Self {
            Self(values.into())
        }
    }
    impl From<Vec<ImmediateSequentialProofInProofPath>> for SequentialProofInProofPath {
        fn from(values: Vec<ImmediateSequentialProofInProofPath>) -> Self {
            Self(values)
        }
    }
    ...
    impl
        From<(
            ImmediateSequentialProofInProofPath,
            ImmediateSequentialProofInProofPath,
        )> for SequentialProofInProofPath
    {
        fn from(
            values: (
                ImmediateSequentialProofInProofPath,
...
pub type SequentialProofInProof<'a, P, Rule> =
    SequentialProofAtPath<'a, P, Rule, SequentialProofInProofPath>;
pub type SequentialProofInProofEnum<'a, P, Rule> =
    SequentialProofAtPathEnum<'a, P, SequentialProofInProofPath, Rule>;

pub type OwnedImmediateProofInProof<P, Rule> =
    OwnedSequentialProofAtPath<P, Rule, SequentialProofInProofPath>;
pub type OwnedImmediateProofInProofEnum<P, Rule> =
    OwnedSequentialProofAtPathEnum<P, SequentialProofInProofPath, Rule>;
