...
pub trait Binder: Sized {
    ...
    #[inline]
    fn get_by_bounds<'binder, B: GetBounds<Self>>(
        &'binder self,
        bounds: &B,
    ...
    #[inline]
    fn get_unique_by_bounds<'binder, B: UniqueGetBounds<Self>>(
        &'binder self,
        bounds: &B,
...
pub trait GetBinder<B: GetBound>: Binder {
    ...
    fn get_with_extra_data<'binder>(
        &'binder self,
        bound: &B,
    ) -> HashSet<(&'binder Self::Value, B::ExtraReturnData)>;

    fn get_intersection<'binder, 'bounds, I: IntoIterator<Item = &'bounds B>>(
        &'binder self,
        bounds: I,
    ) -> HashSet<&'binder Self::Value>
    where
