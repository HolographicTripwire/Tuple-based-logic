...
use crate::utils::collections::maps::conflictless::KeyConflictError;
use crate::utils::traits::combinable::TryCombine;
use crate::utils::traits::try_from_iter::TryFromIterator;

const UNASSIGNED_JUST_INSERTED_VALUE_EXCEPTION: &str =
    "A value was just inserted into HashMap at a particular key, yet the key remains unassigned";
...
impl<K: Eq + Hash, V: PartialEq<V>> ConflictlessHashMap<K, V> {
    ...
    pub fn insert_all<I: IntoIterator<Item = (K, V)>>(
        &mut self,
        iter: I,
...
pub fn insert_into_hashmap_without_conflicts<
    K: Clone + Eq + Hash,
    V: PartialEq<V>,
    I: IntoIterator<Item = (K, V)>,
>(
    mut insert_into: HashMap<K, V>,
    insert_from: I,
) -> Result<HashMap<K, V>, KeyConflictError<K, V>> {
...
pub fn create_hashmap_without_conflicts<
    K: Clone + Eq + Hash,
    V: PartialEq<V>,
    I: IntoIterator<Item = (K, V)>,
>(
...
pub fn combine_hashmaps_without_conflicts<
    K: Clone + Eq + Hash,
    V: PartialEq<V>,
...
>(
    ...
    for map2 in hashmaps {
        let (smaller, larger) = if map1.len() < map2.len() {
            (map1, map2)
        } else {
            (map2, map1)
        };
