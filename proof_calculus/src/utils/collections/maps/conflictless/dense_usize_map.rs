...
use crate::utils::{collections::{iterators::split_into_max_by_key, maps::{conflictless::KeyConflictError, dense_usize_map::{DenseUsizeMap, KEYSET_VALUE_OUT_OF_BOUNDS_EXCEPTION, UNASSIGNED_JUST_INSERTED_VALUE_EXCEPTION}}}, traits::{combinable::TryCombine, try_from_iter::TryFromIterator}};

#[derive(Clone,PartialEq,Eq,Debug)]
pub struct ConflictlessDenseUsizeMap<K: Clone + Eq + Hash + Into<usize>,V: PartialEq<V>>(DenseUsizeMap<K,V>);
...
impl <K: Clone + Eq + Hash + Into<usize>, V: PartialEq<V>> ConflictlessDenseUsizeMap<K,V> {
    ...
    pub fn insert(&mut self, key: K, value: V) -> Result<(),KeyConflictError<K,V>> {
        match self.0.insert(key.clone(),value) {
            Some(old_value) => {
                let new_value = self.0.inner_vec().get_mut(key.clone().into())
...
impl <K: Clone + Eq + Hash + Into<usize>, V: PartialEq<V>> TryCombine for ConflictlessDenseUsizeMap<K,V> {
    ...
    fn try_combine<I: IntoIterator<Item = Self>>(maps: I) -> Result<Self,Self::CombinationError> {
        ...
        for map in remaining {
            for (key, value) in map {
                insert_within_bounds_into_vec_without_conflicts(&mut largest.0.inner_vec(), key, value)?;
            }
        }
...
impl <K: Clone + Eq + Hash + Into<usize>, V: PartialEq<V>> TryFromIterator<(K,V)> for ConflictlessDenseUsizeMap<K,V> {
...
impl <K: Clone + Eq + Hash + Into<usize>, V: PartialEq<V>> TryFrom<Vec<(K,V)>> for ConflictlessDenseUsizeMap<K,V> {
    type Error = KeyConflictError<K,V>;
...
impl <K: Clone + Eq + Hash + Into<usize>, V: PartialEq<V>> IntoIterator for ConflictlessDenseUsizeMap<K,V> {
    type Item = (K,V);
