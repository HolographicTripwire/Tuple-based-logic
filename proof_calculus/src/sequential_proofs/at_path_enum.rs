...
use crate::sequential_proofs::{Proof, composite::CompositeSequentialProof, inference::{TblInference, InferenceRule}};

pub enum ProofAtPathEnum<'a,Path,Rule: InferenceRule> {
    Inference(ObjAtPath<'a,TblInference<Rule>,Path>),
...
pub enum OwnedProofAtPathEnum<Path,Rule:InferenceRule> {
    Inference(OwnedObjAtPath<TblInference<Rule>,Path>),
