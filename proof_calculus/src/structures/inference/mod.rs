...
use crate::structures::{Proposition, propositions::{ParentOfAssumptions, ParentOfExplicitConclusions, paths::{AssumptionIProofStepPath, ExplicitConclusionInProofStepPath}}, sequential_proofs::subproof::ProofInProofPath};
...
impl <P: Proposition, Rule: InferenceRule<P>> ParentOfAssumptions<P> for Inference<P, Rule> {
    fn get_assumption_paths(&self) -> impl IntoIterator<Item = AssumptionIProofStepPath>
        { (0..self.assumptions.len()).map(|n| AssumptionIProofStepPath::from(n)) }
    ...
    fn into_located_assumptions_owned(self) -> impl IntoIterator<Item = OwnedObjAtPath<P,AssumptionIProofStepPath>> where Self:Sized {
        self.assumptions.into_iter()
            .enumerate()
            .map(|(id,conclusion)| OwnedObjAtPath{obj: conclusion, path: AssumptionIProofStepPath::from(id)})
    }
}
