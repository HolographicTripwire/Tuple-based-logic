...
use crate::{structures::{propsets::{Dnf, DnfClause}, propositions::Proposition}, utils::{collections::tiny_immutable_ordered_set::TinyImmutableOrderedSet, traits::fast_ord::FastOrd}};

#[derive(Clone,PartialEq,Eq,Hash,Debug)]
pub struct TinyDnf<P: Proposition + FastOrd>(TinyImmutableOrderedSet<TinyDnfClause<P>>);
...
impl <P: Proposition + FastOrd> TinyDnf<P> {
    pub fn new(clauses: Box<[TinyDnfClause<P>]>) -> Self {
        Self ( TinyImmutableOrderedSet::new(clauses) )
    }
...
impl <P: Proposition + FastOrd> Deref for TinyDnf<P> {
    type Target = TinyImmutableOrderedSet<TinyDnfClause<P>>;
...
pub struct TinyDnfClause<P: Proposition + FastOrd>(TinyImmutableOrderedSet<P>);
...
impl <P: Proposition + FastOrd> TinyDnfClause<P> {
    pub fn new(propositions: Box<[P]>) -> Self { 
        Self(TinyImmutableOrderedSet::new(propositions))
    }
