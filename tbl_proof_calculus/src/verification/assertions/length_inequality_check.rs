...
pub fn assert_expression_length_inequality<'a,C: CompoundTblExpression, Path: Clone>(exprs: &[&'a TblExpressionAtPath<'a,C,Path>]) -> Result<(), ExpressionLengthInequalityError<C,Path>> {
...
/// Check that the provided [Expressions](ExpressionInInference) have inequal length, returning an error otherwise
pub fn assert_fixed_length_expression_length_inequality<'a,const N: usize, C: CompoundTblExpression, Path: Clone>(exprs: &[&'a TblExpressionAtPath<'a,C,Path>; N]) -> Result<(), FixedLengthExpressionLengthInequalityError<N,C,Path>> {
