use proof_calculus::{propositions::collections::binders::{GetPropByIdentityBinder, GetPropsBinder, GetPropsSubsumedByUpropBinder, InsertPropBinder, PropBinder, unassigned::GetBinderForPropSubsumedByUprop}, structures::propositions::collections::trackers::BindGetPropositionsByBounds, utils::collections::multimap::MultiMap};

use std::{collections::{HashMap, HashSet}, hash::Hash};
use crate::{generation::{expressions::{UnassignedTblExpression, at_path_enum::UnassignedTblSubexpressionInExpressionEnum, compound::UnassignedCompoundTblExpression, subexpressions::{UnassignedTblSubexpressionInExpression, immediate::LocatedParentOfImmediateUnassignedSubexpressions as _}, variable::TblExpressionVariable}, proof_calculus_derived::propositions::UnassignedTblProposition}, structures::{expressions::{RcTblExpression, TblExpression, at_path_enum::{TblExpressionAtPathEnum, TblSubexpressionInExpressionEnum}, bounds::{TblExpressionBoundAtomExactValue, TblExpressionBoundCompoundExactLength, atom::TblPropositionBoundAtomExactValue, compound::TblPropositionBoundCompoundExactLength, duplication::TblPropositionBoundValueDuplicated}, collections::tracker::bounds::{TblExpressionTrackerBoundsAtomExactValue, TblExpressionTrackerCompoundLengthBounds, TblExpressionTrackerDuplicationBounds}, compound::{CompoundTblExpression, rc::RcCompoundTblExpression}, subexpressions::{LocatedParentOfSubexpressions, TblSubexpressionInExpression, TblSubexpressionInExpressionPath, immediate::LocatedParentOfImmediateSubexpressions}}, proof_calculus_derived::aliases::propositions::TblProposition}};
...
impl <T: Hash + Eq + Clone> PropBinder for TblPropositionTracker<T> {
    type Value = T;
    
    #[inline]
    fn get_all<'a>(&'a self) -> HashSet<&'a Self::Value> { self.get_unbounded_path_matches(&TblSubexpressionInExpressionPath::default()) }
}
impl <T: Hash + Eq + Clone> GetPropsBinder<TblPropositionBoundAtomExactValue> for TblPropositionTracker<T> {
    #[inline]
    fn get<'a>(&'a self, key: &TblPropositionBoundAtomExactValue) -> HashSet<&'a Self::Value>
        { self.atom_value_bounds.get2(&key) }
}
impl <T: Hash + Eq + Clone> GetPropsBinder<TblPropositionBoundCompoundExactLength> for TblPropositionTracker<T> {
    #[inline]
    fn get<'a>(&'a self, key: &TblPropositionBoundCompoundExactLength) -> HashSet<&'a Self::Value>
        { self.compound_length_bounds.get2(&key) }
}
impl <T: Hash + Eq + Clone> GetPropsBinder<TblPropositionBoundValueDuplicated> for TblPropositionTracker<T> {
    #[inline]
    fn get<'a>(&'a self, key: &TblPropositionBoundValueDuplicated) -> HashSet<&'a Self::Value>
        { self.duplicate_value_bounds.get2(&key) }
}

impl <C: CompoundTblExpression, T: Hash + Eq + Clone> GetPropByIdentityBinder<TblProposition<C>> for TblPropositionTracker<T> {
    type DefaultIdentityBounds<'a> = usize where TblProposition<C>: 'a;
}
impl <C: UnassignedCompoundTblExpression, T: Hash + Eq + Clone> GetPropsSubsumedByUpropBinder<UnassignedTblProposition<C>> for TblPropositionTracker<T> {
    type DefaultSubsumedByBounds<'a> = usize where TblProposition<C>: 'a;
}

impl <C: CompoundTblExpression, T: Hash + Eq + Clone> InsertPropBinder<TblProposition<C>> for TblPropositionTracker<T> {
    type DefaultInsertionBounds<'a> = usize where TblProposition<C>: 'a;
    fn insert<'a, Bs: proof_calculus::propositions::bounds::InsertBoundsForProp<'a,TblProposition<C>,Self,Self::Value>>(&'a mut self, bounds: Bs, value: Self::Value) where TblProposition<C>: 'a {
        todo!()
    }
}
