use proof_calculus::propositions::types::{
    assigned::Proposition, unassigned::UnassignedProposition,
};
    ...
    fn partial_assign(self, assignment: &Self::PartialAssignment) -> Self {
        match self {
            ...
            UnassignedTblExpression::Compound(compound) => {
                UnassignedTblExpression::Compound(compound.partial_assign(assignment))
            }
            UnassignedTblExpression::Variable(var) => {
                if let Some(a) = assignment.0.get(var) {
                    a.clone()
                } else {
                    self
                }
    ...
    fn partial_reverse_assign(&self, assigned: &Self) -> Result<Self::PartialAssignment, ()> {
        match (&self, assigned) {
            (UnassignedTblExpression::Atom(a1), UnassignedTblExpression::Atom(a2)) => {
                if a1 == a2 {
                    Ok(PartialTblExpressionAssignment::default())
                } else {
                    Err(())
                }
            }
            ...
            (UnassignedTblExpression::Variable(var), other) => Ok(
                PartialTblExpressionAssignment::from([(*var, other.clone())]),
            ),
            _ => Err(()),
        }
    }
