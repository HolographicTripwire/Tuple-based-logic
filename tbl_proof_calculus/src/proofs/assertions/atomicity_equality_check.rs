...
pub fn assert_expression_atomicity_equality<'a, C: TblExpressionCompound, Path: Clone>(
    exprs: &[&'a TblExpressionAtPath<'a, C, Path>],
) -> Result<bool, ExpressionAtomicityEqualityError<C, Path>> {
    let mut iter = exprs.iter().map(|o| o.obj.is_atom());
    let first_atomicity = iter
        .next()
        .expect("Cannot check unitarity equality for zero expressions");
    for nth_atomicity in iter {
        if nth_atomicity != first_atomicity {
            return Err(ExpressionAtomicityEqualityError {
...
pub struct FixedLengthExpressionAtomicityEqualityError<
    const N: usize,
...
pub fn assert_fixed_length_expression_atomicity_equality<'a, const N: usize, C, Path>(
    exprs: &[&'a TblExpressionAtPath<'a, C, Path>; N],
) -> Result<bool, FixedLengthExpressionAtomicityEqualityError<N, C, Path>>
where
    C: TblExpressionCompound,
    Path: Clone,
{
    if N == 0 {
        panic!("Cannot check atomicity equality for zero expressions")
    }
