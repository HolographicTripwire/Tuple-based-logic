...
pub fn assert_expression_value_inequality<'a, C: TblExpressionCompound, Path: Clone>(
    exprs: &[&'a TblExpressionAtPath<'a, C, Path>],
) -> Result<(), ExpressionValueInequalityError<C, Path>> {
    let iter = exprs.iter().map(|o| o.obj);
    let mut values = HashSet::new();
    for value in iter {
        if !values.insert(value) {
            return Err(ExpressionValueInequalityError {
...
pub fn assert_fixed_length_expression_value_inequality<
    'a,
    const N: usize,
...
>(
...
) -> Result<(), FixedLengthExpressionValueInequalityError<N, C, Path>> {
    if N == 0 {
    ...
    for value in iter {
        if !values.insert(value) {
            return Err(FixedLengthExpressionValueInequalityError {
