...
use crate::expressions::types::assigned::{TblExpression, TblExpressionAtPath, atom::OwnedTblExpressionAtomAtPath, compound::{TblExpressionCompound, OwnedTblExpressionCompoundAtPath}};

#[derive(Clone,PartialEq,Eq,Hash,Debug)]
pub struct ExpressionCheckAtomicError<C: TblExpressionCompound, Path>(pub OwnedTblExpressionCompoundAtPath<C,Path>);
...
pub fn assert_expression_atomic<'a,C: TblExpressionCompound, Path: Clone>(expr: &TblExpressionAtPath<'a,C,Path>) -> Result<(), ExpressionCheckAtomicError<C,Path>> {
    if let TblExpression::Compound(c) = expr.obj { Err(ExpressionCheckAtomicError(
        OwnedObjAtPath { obj: c.clone(), path: expr.path.clone() }
    ))} else { Ok(()) }
}
...
impl <C: TblExpressionCompound,Path> ExpressionAtomicityCheckError<C,Path> {
    ...
    pub fn into_expression(self) -> TblExpression<C> { match self {
        ExpressionAtomicityCheckError::ExpectedCompoundFoundAtom(atom) => TblExpression::Atom(atom.obj),
        ExpressionAtomicityCheckError::ExpectedAtomFoundCompound(compound) => TblExpression::Compound(compound.obj),
    } }
}
...
C: TblExpressionCompound, Path: Clone {
    match expr.obj {
        TblExpression::Atom(atom) => {
            ...
            else { Err(ExpressionAtomicityCheckError::ExpectedCompoundFoundAtom(OwnedObjAtPath{obj: *atom, path: expr.path.clone()})) }
        }, TblExpression::Compound(compound) => {
            if expected_atomicity == false { Ok(()) }
            else { Err(ExpressionAtomicityCheckError::ExpectedAtomFoundCompound(OwnedObjAtPath{obj: compound.clone(), path: expr.path.clone()})) }
        }
    }
}
