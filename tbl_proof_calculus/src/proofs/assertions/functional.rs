...
use crate::{
    expressions::{
        paths::immediate::ImmediateTblSubexpressionInExpressionPath,
        types::assigned::{
    ...
    proofs::assertions::{
        ExpressionCheckInatomicError, ExpressionLengthCheckError, ExpressionValueCheckError,
        assert_expression_value, expression_as_slice,
    },
};

#[derive(Clone)]
pub enum UnwrapInvocationExpressionError<
    C1: TblExpressionCompound,
    Path: Clone,
    Path2,
    C2: TblExpressionCompound,
> {
...
pub fn unwrap_invocation_expression<'a, C1, Path, Path2, C2>(
    invocation: &'a ObjAtPath<'a, TblExpression<C1>, Path>,
    expected_head: &TblExpression<C2>,
) -> Result<
    Box<[ObjAtPath<'a, TblExpression<C1>, Path2>]>,
    UnwrapInvocationExpressionError<C1, Path, Path2, C2>,
>
where
    C1: TblExpressionCompound + PartialEq<C2>,
    C2: TblExpressionCompound + PartialEq<C1>,
    Path: Clone,
    Path2: Clone + From<(Path, ImmediateTblSubexpressionInExpressionPath)>,
{
    ...
    let (head, tail) = vec
        .split_first()
        .ok_or(UnwrapInvocationExpressionError::NoFirstElement(
            invocation.clone().into(),
        ))?;
...
pub fn unwrap_fixed_length_invocation_expression<'a, const N: usize, C1, Path, Path2, C2>(
    invocation: &'a TblExpressionAtPath<'a, C1, Path>,
    expected_head: &TblExpression<C2>,
) -> Result<
    Box<[TblExpressionAtPath<'a, C1, Path2>; N]>,
    Either<
        UnwrapInvocationExpressionError<C1, Path, Path2, C2>,
        ExpressionLengthCheckError<C1, Path>,
    >,
>
where
    C1: TblExpressionCompound + PartialEq<C2>,
...
{
    let tail =
        unwrap_invocation_expression(invocation, expected_head).map_err(|err| Either::Left(err))?;
    ...
    let x: Box<[TblExpressionAtPath<'a, C1, Path2>; N]> = match y.try_into() {
        Ok(b) => b,
        Err(_) => {
            return Err(Either::Right(ExpressionLengthCheckError {
                expected_length: N,
                expression: invocation.clone().into(),
            }));
        }
    };
    Ok(x)
