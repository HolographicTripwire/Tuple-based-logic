...
impl<C: TblExpressionCompound, Path> ExpressionLengthCheckError<C, Path> {
    pub fn get_actual_length(&self) -> TblExpressionLength {
        self.expression.obj.len()
    }
...
pub fn assert_expression_length<'a, C: TblExpressionCompound, Path: Clone>(
    expr: &TblExpressionAtPath<'a, C, Path>,
    expected_length: usize,
) -> Result<(), ExpressionLengthCheckError<C, Path>> {
    match expr.obj {
        ...
        TblExpression::Compound(compound) => {
            if compound.len() == expected_length {
                Ok(())
            } else {
                Err(ExpressionLengthCheckError {
                    expected_length,
                    expression: expr.clone().into(),
                })
            }
