...
use crate::expressions::types::assigned::{OwnedTblExpressionAtPath, TblExpression, atom::{TblExpressionAtomAtPath, OwnedTblExpressionAtomAtPath}, compound::{TblExpressionCompound, TblExpressionCompoundAtPath, OwnedTblExpressionCompoundAtPath}};
...
impl <'a,C:TblExpressionCompound,Path> TblExpressionAtPathEnum<'a,C,Path> {
    pub fn is_atom(&self) -> bool { if let TblExpressionAtPathEnum::Atom(_) = self { true } else { false } }
    pub fn is_compound(&self) -> bool { if let TblExpressionAtPathEnum::Atom(_) = self { true } else { false } }
}
impl <'a,C:TblExpressionCompound,Path> TryInto<TblExpressionAtomAtPath<'a,Path>> for TblExpressionAtPathEnum<'a,C,Path> {
    type Error = ();
...
impl <'a,C:TblExpressionCompound,Path> TryInto<TblExpressionCompoundAtPath<'a,C,Path>> for TblExpressionAtPathEnum<'a,C,Path> {
    type Error = ();
...
impl <'a,C: TblExpressionCompound, Path> From<ObjAtPath<'a,TblExpression<C>,Path>> for TblExpressionAtPathEnum<'a,C,Path> {
    fn from(value: ObjAtPath<'a,TblExpression<C>,Path>) -> Self { match value.obj {
        TblExpression::Atom(atom) => Self::Atom(ObjAtPath { obj: atom, path: value.path }),
...
impl <C:TblExpressionCompound,Path> TryInto<OwnedTblExpressionAtomAtPath<Path>> for OwnedTblExpressionAtPathEnum<C,Path> {
    type Error = ();
...
impl <C:TblExpressionCompound,Path> TryInto<OwnedTblExpressionCompoundAtPath<C,Path>> for OwnedTblExpressionAtPathEnum<C,Path> {
    type Error = ();
...
impl <C: TblExpressionCompound, Path> From<OwnedTblExpressionAtPath<C,Path>> for OwnedTblExpressionAtPathEnum<C,Path> {
    fn from(value: OwnedObjAtPath<TblExpression<C>,Path>) -> Self { match value.obj {
        TblExpression::Atom(atom) => Self::Atomic(OwnedObjAtPath { obj: atom, path: value.path }),
        TblExpression::Compound(compound) => Self::Compound(OwnedObjAtPath { obj: compound, path: value.path }),
    }}
}
impl <C: TblExpressionCompound,Path> Into<OwnedTblExpressionAtPath<C,Path>> for OwnedTblExpressionAtPathEnum<C,Path> {
    fn into(self) -> OwnedObjAtPath<TblExpression<C>, Path> { match self {
        Self::Atomic(inner) => OwnedObjAtPath { obj: TblExpression::Atom(inner.obj), path: inner.path },
