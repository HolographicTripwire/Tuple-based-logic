...
use crate::expressions::{paths::TblSubexpressionInExpressionPath, types::assigned::{atom::TblExpressionAtom, binding::{binders::{get_helper, insert_helper}, bounds::{TblExpressionBoundAtomExactValue, TblExpressionBoundAtomExistsAtLocation}}}};

pub struct TblExpressionBinderAtomExactValue<T: Hash + Eq>(HashMap<TblSubexpressionInExpressionPath,MultiMap<TblExpressionAtom,T>>);
...
impl <T: Eq + Hash> GetBinder<TblExpressionBoundAtomExistsAtLocation> for TblExpressionBinderAtomExactValue<T> {
    ...
    fn get_with_extra_data<'binder>(&'binder self, bound: &TblExpressionBoundAtomExistsAtLocation) -> HashSet<(&'binder Self::Value,TblExpressionAtom)> {
        match self.0.get(&bound.path) {
...
impl <T: Hash + Eq> TblExpressionBinderAtomExactValue<T> {
    fn get_inner(&self, path: &TblSubexpressionInExpressionPath, value: &TblExpressionAtom) -> HashSet<&T>
        { get_helper(&self.0, path, value) }
    
    pub fn insert(&mut self, path: &TblSubexpressionInExpressionPath, atom: TblExpressionAtom, value: T) -> bool 
