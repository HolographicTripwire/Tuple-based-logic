...
#[derive(Default)]
struct ImportantPathsConstructor(
    HashMap<TblSubexpressionInExpressionPath, ImmediateTblSubexpressionInExpressionPath>,
);
impl ImportantPathsConstructor {
    fn try_insert(&mut self, mut path: TblSubexpressionInExpressionPath) -> bool {
        if let Some(last) = path.0.pop() {
            let needs_insert = self.0.get(&path).map(|inner| *inner < last).unwrap_or(true);
            if needs_insert {
    ...
    fn contains(&self, path: &TblSubexpressionInExpressionPath) -> bool {
        if let Some((last, front)) = path.0.split_last() {
            self.0
                .get(&TblSubexpressionInExpressionPath(
