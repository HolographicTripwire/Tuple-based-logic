...
impl TblExpressionBoundValueDuplicated {
    pub fn new(
        ...
        if path1.fast_cmp(&path2).is_lt() {
            (Self { path1, path2 }, false)
        } else {
            (
                Self {
                    path1: path2,
    ...
    pub fn path1(&self) -> &TblSubexpressionInExpressionPath {
        &self.path1
    }
    pub fn path2(&self) -> &TblSubexpressionInExpressionPath {
        &self.path2
    }
    pub fn into_paths(
        self,
...
impl GetBound for TblExpressionBoundValueDuplicated {
