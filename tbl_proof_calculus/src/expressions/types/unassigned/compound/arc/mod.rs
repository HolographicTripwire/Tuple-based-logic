...
use crate::expressions::types::{assigned::{ArcTblExpression, TblExpression, compound::{TblExpressionCompound, arc::ArcTblExpressionCompound}}, unassigned::{ArcUnassignedTblExpression, UnassignedTblExpression, compound::{arc::UnassignedArcCompoundTblExpression, r#box::UnassignedBoxCompoundTblExpression, rc::UnassignedRcCompoundTblExpression}, variable::TblExpressionVariable}};
    ...
    impl TryInto<ArcTblExpressionCompound> for &UnassignedArcCompoundTblExpression {
        ...
        fn try_into(self) -> Result<ArcTblExpressionCompound, Self::Error> { self.0.iter()
            .map(|v| <&ArcUnassignedTblExpression as TryInto<ArcTblExpression>>::try_into(v))
            .try_collect()
        }
    }

    impl <const N: usize> From<[UnassignedTblExpression<Self>;N]> for UnassignedArcCompoundTblExpression
        { fn from(exprs: [UnassignedTblExpression<Self>;N]) -> Self { Self(Arc::new(exprs)) } }
    ...
    impl From<Rc<[UnassignedTblExpression<Self>]>> for UnassignedArcCompoundTblExpression
        { fn from(exprs: Rc<[UnassignedTblExpression<Self>]>) -> Self { Self(Arc::from(exprs.into_iter().as_slice())) } }
    impl From<Arc<[UnassignedTblExpression<Self>]>> for UnassignedArcCompoundTblExpression
        { fn from(exprs: Arc<[UnassignedTblExpression<Self>]>) -> Self { Self(exprs) } }
        ...
        fn from(value: &UnassignedBoxCompoundTblExpression) -> Self {
            value.0.iter()
                .map(|i| match i {
                    ...
                    UnassignedTblExpression::Compound(compound) => UnassignedTblExpression::Compound(UnassignedArcCompoundTblExpression::from(compound)),
                })
                .collect()
        }
    }
