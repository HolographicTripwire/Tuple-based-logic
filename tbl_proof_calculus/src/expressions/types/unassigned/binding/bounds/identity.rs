...
impl UnassignedTblExpressionIdentityBound {
    fn path(&self) -> &TblSubexpressionInExpressionPath {
        match self {
            UnassignedTblExpressionIdentityBound::AtomValue(atom_bound) => &atom_bound.path,
            UnassignedTblExpressionIdentityBound::VariableValue(variable_bound) => {
                &variable_bound.path
            }
            UnassignedTblExpressionIdentityBound::CompoundLength(compound_bound) => {
                &compound_bound.path
            }
        }
    }
}
impl GetBound for UnassignedTblExpressionIdentityBound {
