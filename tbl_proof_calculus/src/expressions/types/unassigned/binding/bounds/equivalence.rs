...
use crate::expressions::{
    ...
    types::{
        ...
        unassigned::binding::bounds::UnassignedTblExpressionBoundVariableExistsAtLocation,
    },
};
...
impl UnassignedTblExpressionEquivalenceBound {
    fn path(
        ...
        match self {
            UnassignedTblExpressionEquivalenceBound::AtomValue(atom_bound) => {
                (&atom_bound.path, None)
            }
            UnassignedTblExpressionEquivalenceBound::Variable(variable_bound) => {
                (&variable_bound.path, None)
            }
            UnassignedTblExpressionEquivalenceBound::CompoundLength(compound_bound) => {
                (&compound_bound.path, None)
            }
            UnassignedTblExpressionEquivalenceBound::ValueDuplicated(duplication_bound) => {
                (duplication_bound.path1(), Some(duplication_bound.path2()))
            }
        }
    }
}
impl GetBound for UnassignedTblExpressionEquivalenceBound {
...
impl From<UnassignedTblExpressionBoundVariableExistsAtLocation>
    for UnassignedTblExpressionEquivalenceBound
{
