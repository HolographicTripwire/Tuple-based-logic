...
use crate::expressions::{
    ...
    types::{
        ...
        unassigned::{
            compound::{
                UnassignedTblExpressionCompound, arc::UnassignedArcCompoundTblExpression,
                r#box::UnassignedBoxCompoundTblExpression, rc::UnassignedRcCompoundTblExpression,
            },
            subexpressions::{
                ParentOfUnassignedSubexpressions, UnassignedTblSubexpressionInExpression,
                immediate::ParentOfImmediateUnassignedSubexpressions,
                iterators::depth_first::counterclockwise::{
                    CounterclockwiseDepthFirstLocatedUnassignedTblSubexpressionIterator,
                    CounterclockwiseDepthFirstUnassignedTblSubexpressionIterator,
                },
            },
...
pub type UnassignedTblExpressionAtPath<'a, C: UnassignedTblExpressionCompound, Path> =
    ObjAtPath<'a, UnassignedTblExpression<C>, Path>;
pub type BoxUnassignedTblExpressionAtPath<'a, Path> =
    UnassignedTblExpressionAtPath<'a, UnassignedBoxCompoundTblExpression, Path>;
pub type RcUnassignedTblExpressionAtPath<'a, Path> =
    UnassignedTblExpressionAtPath<'a, UnassignedRcCompoundTblExpression, Path>;
pub type ArcUnassignedTblExpressionAtPath<'a, Path> =
    UnassignedTblExpressionAtPath<'a, UnassignedArcCompoundTblExpression, Path>;

pub type OwnedUnassignedTblExpressionAtPath<C: UnassignedTblExpressionCompound, Path> =
    OwnedObjAtPath<UnassignedTblExpression<C>, Path>;
pub type OwnedBoxUnassignedTblExpressionAtPath<Path> =
    OwnedUnassignedTblExpressionAtPath<UnassignedBoxCompoundTblExpression, Path>;
pub type OwnedRcUnassignedTblExpressionAtPath<Path> =
    OwnedUnassignedTblExpressionAtPath<UnassignedRcCompoundTblExpression, Path>;
pub type OwnedArcUnassignedTblExpressionAtPath<Path> =
    OwnedUnassignedTblExpressionAtPath<UnassignedArcCompoundTblExpression, Path>;

impl<C: UnassignedTblExpressionCompound> UnassignedTblExpression<C> {
    pub fn replace(
        &self,
        to_replace: &UnassignedTblExpression<C>,
        replace_with: &UnassignedTblExpression<C>,
    ) -> Self {
        if self == to_replace {
            replace_with.clone()
    ...
    pub fn is_atom(&self) -> bool {
        if let UnassignedTblExpression::Atom(_) = self {
            true
        } else {
            false
        }
    ...
    pub fn is_compound(&self) -> bool {
        if let UnassignedTblExpression::Compound(_) = self {
            true
        } else {
            false
        }
    ...
    pub fn is_variable(&self) -> bool {
        if let UnassignedTblExpression::Variable(_) = self {
            true
        } else {
            false
        }
    ...
    pub fn get_subexpressions_helper(
        ...
        index: usize,
    ) -> Result<&UnassignedTblExpression<C>, ()> {
    ...
    pub fn as_slice(&self) -> Result<&[UnassignedTblExpression<C>], ()> {
        match self {
            UnassignedTblExpression::Compound(proposition_exprs) => {
                Ok(proposition_exprs.as_slice())
            }
            _ => Err(()),
        }
    }
...
impl<C: UnassignedTblExpressionCompound> ParentOfImmediateUnassignedSubexpressions<C>
    for UnassignedTblExpression<C>
{
    fn get_immediate_subexpression_paths(
        &self,
    ) -> impl IntoIterator<Item = ImmediateTblSubexpressionInExpressionPath> {
        match self {
            UnassignedTblExpression::Compound(compound) => compound
...
impl<C: UnassignedTblExpressionCompound> ParentOfUnassignedSubexpressions<C>
    for UnassignedTblExpression<C>
{
    ...
    fn get_subexpressions<'a>(&'a self) -> impl IntoIterator<Item = &'a UnassignedTblExpression<C>>
    where
        UnassignedTblExpression<C>: 'a,
    {
        CounterclockwiseDepthFirstUnassignedTblSubexpressionIterator::new(self)
    }
    fn get_located_subexpressions<'a>(
        &'a self,
    ) -> impl IntoIterator<Item = UnassignedTblSubexpressionInExpression<'a, C>>
    where
        UnassignedTblExpression<C>: 'a,
    {
...
mod from {
    ...
    use crate::expressions::types::{
        assigned::{TblExpression, atom::TblExpressionAtom, compound::TblExpressionCompound},
        unassigned::{
    ...
    impl<C: TblExpressionCompound, UC: for<'a> From<&'a C> + UnassignedTblExpressionCompound>
        From<&TblExpression<C>> for UnassignedTblExpression<UC>
    {
        fn from(value: &TblExpression<C>) -> Self {
            match value {
                TblExpression::Atom(atom) => UnassignedTblExpression::Atom(*atom),
                TblExpression::Compound(compound) => {
                    UnassignedTblExpression::Compound(compound.into())
                }
            }
        }
    }
    impl<C: TblExpressionCompound, UC: UnassignedTblExpressionCompound> TryInto<TblExpression<C>>
        for &UnassignedTblExpression<UC>
    where
        for<'a> &'a UC: TryInto<C, Error = TblExpressionVariable>,
    {
        type Error = TblExpressionVariable;
        fn try_into(self) -> Result<TblExpression<C>, Self::Error> {
            match self {
                UnassignedTblExpression::Atom(atom) => Ok(TblExpression::Atom(*atom)),
                UnassignedTblExpression::Compound(compound) => {
                    Ok(TblExpression::Compound(compound.try_into()?))
                }
    ...
    impl<
        C1: UnassignedTblExpressionCompound,
        C2: UnassignedTblExpressionCompound + for<'a> From<&'a C1>,
    > From<&UnassignedTblExpression<C1>> for UnassignedTblExpression<C2>
    {
        fn from(value: &UnassignedTblExpression<C1>) -> Self {
            match value {
    ...
    impl<UC: UnassignedTblExpressionCompound> From<UC> for UnassignedTblExpression<UC> {
        fn from(expr: UC) -> Self {
            Self::Compound(expr)
        }
    }
    impl<const N: usize, C: UnassignedTblExpressionCompound> From<[Self; N]>
        for UnassignedTblExpression<C>
    where
        C: From<[Self; N]>,
    {
    ...
    impl<UC: UnassignedTblExpressionCompound> From<Box<[Self]>> for UnassignedTblExpression<UC>
    where
        UC: From<Box<[Self]>>,
    {
    ...
    impl<UC: UnassignedTblExpressionCompound> From<Rc<[Self]>> for UnassignedTblExpression<UC>
    where
        UC: From<Rc<[Self]>>,
    {
    ...
    impl<UC: UnassignedTblExpressionCompound> From<Arc<[Self]>> for UnassignedTblExpression<UC>
    where
        UC: From<Arc<[Self]>>,
    {
    ...
    impl<UC: UnassignedTblExpressionCompound> From<Vec<Self>> for UnassignedTblExpression<UC>
    where
        UC: From<Vec<Self>>,
    {
    ...
    impl<UC: UnassignedTblExpressionCompound + FromIterator<Self>> FromIterator<Self>
        for UnassignedTblExpression<UC>
    {
        fn from_iter<T: IntoIterator<Item = Self>>(iter: T) -> Self {
            Self::Compound(UC::from_iter(iter))
        }
    }
}
