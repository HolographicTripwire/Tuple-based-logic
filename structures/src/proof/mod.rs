...
pub use proposition_in_step_path::*;
pub use step_path::*;

use crate::{inference::{Inference, InferenceRule}, expressions::Proposition};

pub trait ProofStep<'a, Rule:'a + InferenceRule> : HasChildren<'a,PropositionInProofStepPath,Proposition> + HasChildren<'a,AtomicSubproofPath,Proof<Rule>> {
    ...
    fn get_located_assumptions(&'a self) -> impl IntoIterator<Item = ObjAtPath<'a,Proposition,PropositionInProofStepPath>>
    ...
    fn get_located_explicit_conclusions(&'a self) -> impl IntoIterator<Item = ObjAtPath<'a,Proposition,PropositionInProofStepPath>>
...
pub (crate) fn valid_primitive_paths_inner<'a,Rule: 'a + InferenceRule, Step: ProofStep<'a,Rule>>(step: &'a Step) -> impl IntoIterator<Item = PropositionInProofStepPath> {
    let assumptions = (0..step.assumption_paths().into_iter().count()).map(|ix| PropositionInProofStepPath::assumption(ix));
    let conclusions = (0..step.get_conclusions().len()).map(|ix| PropositionInProofStepPath::conclusion(ix));
    assumptions.chain(conclusions)
}
pub (crate) fn get_child_inner<'a,Rule: 'a + InferenceRule, Step: ProofStep<'a,Rule>>(step: &'a Step, path: &PropositionInProofStepPath) -> Result<&'a Proposition,()> {
...
impl <'a, Rule:'a + InferenceRule> HasChildren<'a,PropositionInProofStepPath,Proposition> for Proof<Rule> {
...
impl <'a, Rule:'a + InferenceRule> HasChildren<'a,PropositionInProofStepPath,Proposition> for CompositeProof<Rule> {
    fn valid_primitive_paths(&self) -> impl IntoIterator<Item = PropositionInProofStepPath> { valid_primitive_paths_inner(self) }
