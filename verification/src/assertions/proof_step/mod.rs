...
pub fn assumptions_as_sized_slice<const EXPECTED_SIZE: usize,Rule: InferenceRule>(inference: InferenceInProof<Rule>) -> Result<Box<[OwnedPropositionInProof; EXPECTED_SIZE]>,AssumptionCountCheckError<Rule>> {
...
pub fn explicit_conclusions_as_slice<Rule: InferenceRule>(inference: &InferenceInProof<Rule>) -> Vec<OwnedPropositionInProof> {
    inference.0.get_located_children_owned()
        .into_iter()
        .map(|obj| OwnedPropositionInProof(obj.replace_path(|p| p.into())))
        .collect::<Vec<OwnedPropositionInProof>>()
...
pub fn explicit_conclusions_as_sized_slice<const EXPECTED_SIZE: usize,Rule: InferenceRule>(inference: InferenceInProof<Rule>) -> Result<Box<[OwnedPropositionInProof; EXPECTED_SIZE]>,ExplicitConclusionCountCheckError<Rule>> {
