...
/// Get an [AssessedErrorStringifier] which takes an [Expression](OwnedExpressionInProof) and returns an error message saying that this expression's length is not the expected value
pub fn expression_length_stringifier<'a>(length_expected: usize) -> impl AssessedErrorStringifier<'a,OwnedExpressionInProof,()> {
...
pub fn expression_length_check<'a>(length_expected: usize) -> ErrorStringifiableAssessor<'a,OwnedExpressionInProof,(),()> { ErrorStringifiableAssessor::new(
