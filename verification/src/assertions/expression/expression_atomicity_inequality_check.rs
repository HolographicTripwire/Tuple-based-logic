...
pub fn expression_atomicity_inequality_predicate<'a>() -> impl Assessor<'a,[OwnedExpressionInProof;2],()> {
    move |os: [OwnedExpressionInProof; 2]| { 
        if os[0].0.obj().as_atom().is_ok() != os[1].0.obj().as_atom().is_ok() { Ok(()) }
        else { Err(()) }
    }
}
...
pub fn expression_atomicity_inequality_stringifier<'a>() -> impl AssessedStringifier<'a,[OwnedExpressionInProof;2],()> {
    move |os: [OwnedExpressionInProof; 2],_| format!(
...
pub fn expression_atomicity_inequality_check<'a>() -> StringifiablePredicate<'a,[OwnedExpressionInProof;2],()> { StringifiablePredicate::new(
